...
Így egy karbantarthatóbb, könyebben kezelhető kódbázist kapunk, illetve a hibákat is előbb kitudjuk szúrni, ha megfelelő típusokat vezetünk be és használunk.
...
Amint a fenti \ref{fig:architecture} ábrán látható, a Visual Studio Code szerkesztőn belül helyezkedik el a Visualiser komponens, illetve az Erlang LS kliens implementációja. Az Erlang LS kliens és szerver implementációja a Microsoft Language Server Protocol \cite{microsoftLSp} felett kommunikállnak. A RefactorErl és a Visualiser Web Socket segítségével kommunikálnak, mig az Erlang LS szervere és a RefactorErl RCP (Remote Procedure Call) hívásokkal.
...
\lstset{caption={Típusdefiníciók az \lstinline{els_refactorerl_diagnostics} modulban}, label=src:erlang0elsDiagTypes}
...
\lstset{caption={Elérhető diagnosztikák módosított verziója}, label=src:erlang0avDiagModif}
\begin{lstlisting}[language={erlang}]
...
\lstset{caption={Konfigurációs kulcs, módosított típusdefiníció}, label=src:erlang0ModifyKey}
\begin{lstlisting}[language={erlang}]
-type key()   :: apps_dirs
               | apps_paths
               | capabilities
...
\lstset{caption={Módosított állapot típus}, label=src:erlang0modifyState}
\begin{lstlisting}[language={erlang}]
-type state() :: 
#{ apps_dirs        => [path()]
 , apps_paths       => [path()]
...
\lstset{caption={Konfigurációs kulcsmegkeresése, majd beállítása}, label=src:erlang0elsKeyFind}
\begin{lstlisting}[language={erlang}]
...
...
\lstset{caption={\lstinline{code_action/3} kibővített verziója}, label=src:erlang0codeactmod}
\begin{lstlisting}[language={erlang}]
...
\lstset{caption=\lstinline{els_execute_command_provider:options/0} kibővítése, label=src:erlang0commandprovextend}
\begin{lstlisting}[language={erlang}]
-spec options() -> map().
options() ->
...
\lstset{caption=\lstinline{referl_els} modulbeli típusdefiníció, label=src:erlang0elsTypeDef}
\begin{lstlisting}[language={erlang}]
...
\subsubsection{webSocketHandler.ts}
...
\subsubsection{dependencyGraphView.ts}
...
\lstset{caption=\lstinline{show.erl} teszt fájl, label=testShowERL}
\begin{lstlisting}[language={erlang}]  
-module(show).
...
\lstset{caption=\lstinline{show.erl} teszt fájl diagnosztikai eredményei, label=testShowSHELL} 
...
\lstset{caption=\lstinline{variables.erl} teszt fájl, label=testVariablesERL}
\begin{lstlisting}[language={erlang}]  
-module(variables).
-export([alpha/1]).
...
\lstset{caption=\lstinline{dependency.erl} teszt fájl, label=testdependencyERL}
\begin{lstlisting}[language={erlang}]  
-module(dependency).
...
\lstset{caption=\lstinline{calls.erl} teszt fájl, label=testDynCallERL} 
\begin{lstlisting}[language={erlang}]  
-module(calls).
...
Mivel itt nem a diagnosztikák tartalma a lényeges, hiszen azért a RefactorErl felel, hanem az, hogy amit a RefactorErl küld, az helyesen legyen megjelenítve, ezért \lstinline{els_refactorerl_utils:run_diagnostics/2} függvényt úgy mockoltam, hogy minden esetben ugyanazzal a diagnosztikával térjen vissza.
...
\lstset{caption={Teszt fájlok}, label=src:erlang1}
\begin{lstlisting}[language={erlang}]
alphaMod() -> 
